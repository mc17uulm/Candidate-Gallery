		...
		return (
			<div className="cg_gallery_container">
				{this.props.images.length > 0 ? this.props.images.map((image, index) => (		
						<Candidate image={image} type={this.props.type} index={index} key={`image-${index}`} onDelete={this.props.onDelete} update={(id: string, value: any) => {this.props.update(id, value, index)}} addImage={this.props.addImage} />
